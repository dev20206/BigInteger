...
int compare(struct BigInteger num1, struct BigInteger num2) {
    if (num1.length > num2.length) {
        ...
        int res=0;
        struct node* itr1=num1.l;
        struct node* itr2= num2.l;
        while(itr1!=NULL)
        {
...
struct BigInteger length(struct BigInteger C)
{
    int l=0;
    struct node *temp=C.l;
    while(temp)
    {
        l++;
        temp=temp->next;
...
struct node* inlinkedlist(int n)
{
